...
-- ================================================================
-- BAY THẲNG - Loop set CFrame mỗi frame, tắt hoàn toàn physics
-- ================================================================
local function TweenTo(targetCFrame)
    ...
    local duration = math.max(0.05, dist / 350)
    local elapsed  = 0
    ...
    while elapsed < duration do
        ...
        -- Lerp thẳng từ A đến B, không bị ảnh hưởng gì
...
-- ================================================================
-- SERVER HOP SYSTEM - Ngẫu nhiên hoàn toàn, không trùng server
-- ================================================================
...
local function GetServers()
    ...
    if not ok or not req then return servers end
...
local HopStatusLabel  -- forward declare

local function HopToServer()
    pcall(function()
        if HopStatusLabel then HopStatusLabel.Text = "🌐 Đang lấy danh sách server..." end

        local servers = GetServers()

        -- Hết server chưa thăm → reset toàn bộ
        if #servers == 0 then
            VisitedServers = { [JobId] = true }
            task.wait(1)
            servers = GetServers()
        ...
        if #servers == 0 then
            if HopStatusLabel then HopStatusLabel.Text = "⚠️ Không có server → Rejoin..." end
            task.wait(2)
        ...
        VisitedServers[picked.id] = true

        -- Countdown 5 giây
        for i = 5, 1, -1 do
            if HopStatusLabel then
                HopStatusLabel.Text = string.format("⏳ Hop sau %ds • %d players", i, picked.playing or 0)
            end
            task.wait(1)
        end
        ...
        if not ok then
            if HopStatusLabel then HopStatusLabel.Text = "⚠️ Hop lỗi → Rejoin..." end
            task.wait(1)
            pcall(function() TeleportService:Teleport(PlaceId, LocalPlayer) end)
        end
    end)
...
-- === Server Hop Status ===
HopStatusLabel = Instance.new("TextLabel")  -- gán vào biến forward-declare ở trên
...
-- ================================================================
-- DRAG - dùng RunService để kéo mượt, không conflict với click
-- ================================================================
do
    local dragging = false
    local dragStartMouse = Vector2.new()
    local dragStartFrame = Vector2.new()

    -- Vùng drag: chỉ kéo trên TitleLabel (không phải toàn frame)
    TitleLabel.InputBegan:Connect(function(input)
        ...
        or input.UserInputType == Enum.UserInputType.Touch then
            ...
            dragStartFrame = Vector2.new(pos.X.Offset, pos.Y.Offset)
        end
    end)

    TitleShadow.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1
        or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true
            dragStartMouse = Vector2.new(input.Position.X, input.Position.Y)
    ...
    RunService.Heartbeat:Connect(function()
        ...
        MainFrame.Position = UDim2.new(0, dragStartFrame.X + dx, 0, dragStartFrame.Y + dy)
    end)
end
...
local function MainFindFruitLoop()
    while true do
        pcall(function()
            ...
            if #fruits == 0 then
                HopStatusLabel.Text = "🌐 There are no fruits in the server...."
                FruitInfoLabel.Text = "⚡ Hopping..."
                task.wait(1)
                HopToServer()
                return
            end
            ...
            if not targetFruit then
                HopStatusLabel.Text = "✅ Storage → Đổi server..."
                task.wait(1)
                HopToServer()
                return
            end
            ...
            if targetFruit and targetFruit:FindFirstChild("Handle") then
                ...
                HopStatusLabel.Text = "🌐 Ngon Thíiii..."
                TweenTo(targetFruit.Handle.CFrame)
                -- Snap chính xác vào trái ngay sau khi tween xong để nhặt
                local char = LocalPlayer.Character
                if char and char:FindFirstChild("HumanoidRootPart") and targetFruit.Parent and targetFruit:FindFirstChild("Handle") then
...
HopStatusLabel.Text = "🌐 Server: Fruit Hopping..."
